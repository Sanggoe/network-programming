...
int main(int argc, char *argv[]) {
	...
	pid_t pid;
	unsigned int yes = 1;
	struct sockaddr_in mcast_group;	// 멀키태스트 그룹주소
	struct ip_mreq mreq;
	...
	// fork() 실행 : child는 수신, parent는 송신 담당
	if ((pid = fork()) < 0) {
		printf("error :	fork\n");
		exit(0);
	}
	...
	// parent process : 키보드 입력 및 메시지 송신 담당
		...
		while (fgets(message, MAXLINE, stdin) != NULL) {
			...
			len = strlen(line);
			if (sendto(send_s, line, len, 0, (struct sockaddr*)&mcast_group, sizeof(mcast_group)) < len) {
				printf("error : sendto\n");
				exit(0);
			}
		}
